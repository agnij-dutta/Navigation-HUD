...
import finch.archerhud.ImageUtils;
import finch.archerhud.app.detect.ImageDetectListener;
import finch.archerhud.eOutAngle;
import finch.archerhud.eUnits;
import finch.archerhud.hud.BMWHUD;
import finch.archerhud.hud.DummyHUD;
import finch.archerhud.hud.archerHUD;
import finch.archerhud.hud.HUDInterface;
...
public class MainActivity extends AppCompatActivity {
                ...
                case CONNECTED:
                    runOnUiThread(() -> {
                        if (mHudConnectedSwitch != null) {
                            String hudName = isBMW() ? "Godseye HUD" : "Archer HUD";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        //sdcard/Android/data/finch.archerhud.app/cache/screenshots
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {//Capture the return key.
        if ((keyCode == KeyEvent.KEYCODE_BACK)) {
            //If the return key is pressed, execute the exit confirmation.
            confirmExit();
            return true;
        }
    ...
    private void confirmExit() {
        ...
        ad.setTitle("Exit");
        ad.setMessage("Leave Archer HUD app?");
        //Exit button
        ad.setPositiveButton("Yes", (dialog, i) -> {
            MainActivity.this.finish();//Close activity
        });
        ad.setNegativeButton("No", (dialog, i) -> {
            //If you don't want to exit, no action is needed to be taken.
        });
        ad.show();//Display a dialog box.
    }
